% 3rd version, modified, absolute imaging 
% clear
...
parfor j = 1:N     
   ...
   %%%  Add random noise onto the traning data
    ...
    % intVdata(:,j) = sum(Uel_intV,2); %%% interval voltage
    U_T(:,j) = Uel; %%% voltage training data
end

% Split = 0.8;
% 
% XTrain = sigTdata(:,1:Split*N);
% YTrain = intVdata(:,1:Split*N);
% XValidation = sigTdata(:,Split*N+1:N);
% YValidation = intVdata(:,Split*N+1:N);


% save('D:\matlabwork\crack-liang\neural networks\Re_Crack_Train.mat','XTrain','YTrain')
% save('D:\matlabwork\crack-liang\neural networks\Re_Crack_Validation.mat','XValidation','YValidation')
% save('D:\matlabwork\crack-liang\neural networks\Node_injection_Crack.mat','Node1','Element1','I','z','MeasPatt','II1')